...
static android::CameraHal* gCameraHal;
static android::CameraHalManager* gCamManager = NULL;
...
typedef struct infotm_camera_device {
    camera_device_t base;
    /* TI specific "private" data can go here (base.priv) */
    int cameraid;
} infotm_camera_device_t;
...
{
    int rv = -EINVAL;
    infotm_camera_device_t* infotm_dev = NULL;

    if(!device)
        return rv;

    infotm_dev = (infotm_camera_device_t*) device;
...
int camera_device_close(hw_device_t* device)
{
    ...
    if (infotm_dev) {
        ...
        if (infotm_dev->base.ops) {
            free(infotm_dev->base.ops);
        }
        free(infotm_dev);
    }
done:
                ...
                hw_device_t** device)
{
    int rv = 0;
    int cameraid;
    infotm_camera_device_t* camera_device = NULL;
    ...
    if (name != NULL) {
        ...
        if(cameraid > 1)
        {
            ALOGE("camera service provided cameraid out of bounds, "
                    "cameraid = %d, num supported = 1",
                    cameraid);
            rv = -EINVAL;
            goto fail;
        }

        camera_device = (infotm_camera_device_t*)malloc(sizeof(*camera_device));
        if(!camera_device)
        {
        ...
        *device = &camera_device->base.common;
        ...
        camera = new android::CameraHal(camera_device->cameraid);
...
int camera_get_number_of_cameras(void)
{
	if(gCamManager == NULL)
	{
    	android::CameraHalManager* cameraManager = NULL;
		cameraManager = new android::CameraHalManager();
		gCamManager == cameraManager;
	}
    return gCamManager -> HAL_getNumberOfCameras();
}
...
int camera_get_camera_info(int camera_id, struct camera_info *info)
{
	...
	if((camera_id<0) || (camera_id>1) || info == NULL)
        rv = -EINVAL;
	gCamManager -> HAL_getCameraInfo(camera_id, info);
