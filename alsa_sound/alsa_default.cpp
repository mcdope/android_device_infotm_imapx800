...
module      : 0,
    ...
    */
    ...
    {android_audio_legacy::AudioSystem::DEVICE_OUT_PWMA,           "_Pwma"},
    {android_audio_legacy::AudioSystem::DEVICE_IN_DMIC,           "_Dmic"},
    };
	...
	ALOGV("Set %s PCM format to %s (%s)", streamName(), formatName, formatDesc);
	...
	if (err < 0)
	    ...
	    ALOGV("Set %s sample rate to %u HZ", stream, requestedRate);
